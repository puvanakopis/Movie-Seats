import React from 'react'
import { Link } from 'react-router-dom'
import { assets } from '../assets/assets'

const Footer = () => {

  const handleNav = () => {
    scrollTo(0, 0);
  }

  return (
    <footer className="px-6 pt-8 md:px-16 lg:px-36 mt-40 w-full text-gray-300">
      <div className="flex flex-col md:flex-row justify-between w-full gap-10 border-b border-gray-500 pb-14">

        {/* ----------- Left Section ----------- */}
        <div className="md:max-w-96">
          <img alt="Logo" className="h-7" src={assets.logo} />

          <p className="mt-6 text-sm">
            Quick Show is your trusted platform for booking movie tickets with ease.
            Discover the latest releases, explore nearby theaters, and enjoy a smooth
            and secure ticket-booking experience — all in one place.
          </p>

          <div className="flex items-center gap-2 mt-4">
            <img src={assets.googlePlay} alt="google play" className="h-9 w-auto rounded" />
            <img src={assets.appStore} alt="app store" className="h-9 w-auto rounded" />
          </div>
        </div>

        {/* ----------- Right Section ----------- */}
        <div className="flex-1 flex items-start md:justify-end gap-20 md:gap-40">

          {/* ----------- Company Links ----------- */}
          <div>
            <h2 className="font-semibold mb-5">Company</h2>
            <ul className="text-sm space-y-2">

              <li>
                <Link to="/" onClick={handleNav}>Home</Link>
              </li>

              <li>
                <Link to="/about" onClick={handleNav}>About us</Link>
              </li>

              <li>
                <Link to="/contact" onClick={handleNav}>Contact us</Link>
              </li>

              <li>
                <Link to="/privacy-policy" onClick={handleNav}>Privacy policy</Link>
              </li>

            </ul>
          </div>

          {/* ----------- Contact Details ----------- */}
          <div>
            <h2 className="font-semibold mb-5">Get in touch</h2>
            <div className="text-sm space-y-2">
              <p>+94 71 262 1098</p>
              <p>support@quickshow.com</p>
            </div>
          </div>

        </div>
      </div>

      {/* ----------- Footer Bottom ----------- */}
      <p className="pt-4 text-center text-sm pb-5">
        Copyright {new Date().getFullYear()} ©
        <Link to="/" onClick={handleNav} className="ml-1">PrebuiltUI</Link>.
        All Rights Reserved.
      </p>
    </footer>
  )
}

export default Footer